...
$Globals = @{
    ...
    ApplicationManifest = $( if ($admin) { "$PSScriptRoot\App.Manifest" } else { "" } )
} | ConvertTo-BuildGlobals

$Project = New-Object Microsoft.Build.Evaluation.Project -ArgumentList ("$PSScriptRoot\PSWHost.csproj" ,$Globals,"4.0")

if ( $resources ) {
    write-verbose "Add $Resources to project"
    $RemoveMe = $Project.Items | where-object EvaluatedInclude -eq "main.ps1"
    if ( $RemoveMe ) {
